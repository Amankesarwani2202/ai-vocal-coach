"""Exercise 5.2 — Quarter Note Pulse"""
...
EXERCISE_ID = "26_Exercise_5.2_Quarter_Note_Pulse"
NEXT_PAGE = "pages/27_Exercise_5.3_Dotted_Rhythms.py"
BPM = 90
BARS = 4
BEATS_PER_BAR = 4
...
def _init_state():
    if not st.session_state.get("m52_initialized"):
        st.session_state.m52_phase       = "listen"
        st.session_state.m52_result      = None
        st.session_state.m52_initialized = True


def _reset_state():
    st.session_state.m52_initialized = False
    st.session_state.m52_phase  = "listen"
    st.session_state.m52_result = None
...
def _listen_phase():
    _phase_bar("listen")
    st.markdown(f"### Listen — {BPM} BPM quarter-note pulse")
    ...
    st.caption("Tap your foot on each click before recording.")
    if st.button("Ready to record →", key="m52_to_record"):
        st.session_state.m52_phase = "record"
        st.rerun()


def _record_phase():
    _phase_bar("record")
    st.markdown(f"### Sing 'da' on each beat — {BPM} BPM")
    st.caption("4 bars · one syllable per quarter note.")
    audio_input = st.audio_input("Record", key="m52_rec", label_visibility="hidden")

    col_back, col_go = st.columns(2)
    with col_back:
        if st.button("← Listen again", key="m52_relisten", use_container_width=True):
            st.session_state.m52_phase = "listen"
            st.rerun()
    if audio_input is not None:
        audio_bytes = audio_input.read()
        with col_go:
            if st.button("Analyse →", key="m52_analyse", use_container_width=True):
                with st.spinner("Analysing…"):
                    result = analyze_rhythm_timing(audio_bytes, bpm=BPM, beats_per_bar=BEATS_PER_BAR, total_bars=BARS)
                st.session_state.m52_result = result
                st.session_state.m52_phase  = "result"
                st.rerun()


def _result_phase():
    _phase_bar("result")
    result = st.session_state.m52_result or {}
    ...
    with col_a:
        if st.button("Try again", use_container_width=True, key="m52_retry"):
            _reset_state()
            st.session_state.exercise_stage = "recording"
            st.rerun()
    with col_b:
        if st.button("Save and continue", use_container_width=True, key="m52_save"):
...
def _render_exercise_stage():
    _init_state()
    phase = st.session_state.m52_phase
