...
def _init_state():
    if not st.session_state.get("m55_initialized"):
        st.session_state.m55_phase       = "listen"
        st.session_state.m55_result      = None
        st.session_state.m55_initialized = True


def _reset_state():
    st.session_state.m55_initialized = False
    st.session_state.m55_phase  = "listen"
    st.session_state.m55_result = None
...
def _listen_phase():
    _phase_bar("listen")
    st.markdown(f"### Listen — {BPM} BPM (syncopated feel)")
    st.caption("Emphasis falls on the offbeats (the 'and' of each beat). Listen until you feel the groove.")
    ...
    if st.button("Ready to record →", key="m55_to_record"):
        st.session_state.m55_phase = "record"
        st.rerun()


def _record_phase():
    _phase_bar("record")
    st.markdown(f"### Sing on the offbeats — {BPM} BPM")
    st.caption("Sing 'da' between each click: 1-AND-2-AND-3-AND-4-AND. Land on the 'and'.")
    audio_input = st.audio_input("Record", key="m55_rec", label_visibility="hidden")

    col_back, col_go = st.columns(2)
    with col_back:
        if st.button("← Listen again", key="m55_relisten", use_container_width=True):
            st.session_state.m55_phase = "listen"
            st.rerun()
    if audio_input is not None:
        audio_bytes = audio_input.read()
        with col_go:
            if st.button("Analyse →", key="m55_analyse", use_container_width=True):
                with st.spinner("Analysing…"):
                    result = analyze_rhythm_timing(audio_bytes, bpm=BPM, beats_per_bar=BEATS_PER_BAR, total_bars=BARS)
                st.session_state.m55_result = result
                st.session_state.m55_phase  = "result"
                st.rerun()


def _result_phase():
    _phase_bar("result")
    result = st.session_state.m55_result or {}
    ...
    with col_a:
        if st.button("Try again", use_container_width=True, key="m55_retry"):
            _reset_state()
            st.session_state.exercise_stage = "recording"
            st.rerun()
    with col_b:
        if st.button("Save and continue", use_container_width=True, key="m55_save"):
...
def _render_exercise_stage():
    _init_state()
    phase = st.session_state.m55_phase
