...
EXERCISE_ID = "28_Exercise_5.4_Triplets"
NEXT_PAGE = "pages/29_Exercise_5.5_Syncopation.py"
BPM = 72
BARS = 4
BEATS_PER_BAR = 4
...
def _init_state():
    if not st.session_state.get("m54_initialized"):
        st.session_state.m54_phase       = "listen"
        st.session_state.m54_result      = None
        st.session_state.m54_initialized = True


def _reset_state():
    st.session_state.m54_initialized = False
    st.session_state.m54_phase  = "listen"
    st.session_state.m54_result = None
...
def _listen_phase():
    _phase_bar("listen")
    st.markdown(f"### Listen — {BPM} BPM (triplet feel)")
    st.caption("Three equal notes per beat (tri-po-let). Listen until you feel the 3-against-2.")
    ...
    if st.button("Ready to record →", key="m54_to_record"):
        st.session_state.m54_phase = "record"
        st.rerun()


def _record_phase():
    _phase_bar("record")
    st.markdown(f"### Sing 'tri-po-let' on each beat — {BPM} BPM")
    st.caption("Three evenly-spaced syllables per beat. Feel the rolling groove.")
    audio_input = st.audio_input("Record", key="m54_rec", label_visibility="hidden")

    col_back, col_go = st.columns(2)
    with col_back:
        if st.button("← Listen again", key="m54_relisten", use_container_width=True):
            st.session_state.m54_phase = "listen"
            st.rerun()
    if audio_input is not None:
        audio_bytes = audio_input.read()
        with col_go:
            if st.button("Analyse →", key="m54_analyse", use_container_width=True):
                with st.spinner("Analysing…"):
                    result = analyze_rhythm_timing(audio_bytes, bpm=BPM, beats_per_bar=BEATS_PER_BAR, total_bars=BARS)
                st.session_state.m54_result = result
                st.session_state.m54_phase  = "result"
                st.rerun()


def _result_phase():
    _phase_bar("result")
    result = st.session_state.m54_result or {}
    ...
    with col_a:
        if st.button("Try again", use_container_width=True, key="m54_retry"):
            _reset_state()
            st.session_state.exercise_stage = "recording"
            st.rerun()
    with col_b:
        if st.button("Save and continue", use_container_width=True, key="m54_save"):
            add_score(EXERCISE_ID, score, xp)
            _reset_state()
            reset_exercise_flow()
            st.switch_page(NEXT_PAGE)
    if st.button("View progress", use_container_width=True, key="m54_progress"):
        st.switch_page("pages/1_Dashboard.py")


def _render_exercise_stage():
    _init_state()
    phase = st.session_state.m54_phase
