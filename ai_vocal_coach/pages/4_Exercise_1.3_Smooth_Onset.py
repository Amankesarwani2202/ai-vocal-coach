...
from visualization.charts import plot_waveform, plot_pitch_contour

init_session_state()
display_header("Exercise 1.3: Smooth Onset", "Avoid glottal shocks or breathy starts.")

st.markdown("**Instructions:** Sing a repeated 'La' or 'Ma' on a comfortable pitch.")
audio_val = st.audio_input("Record your onsets")

if audio_val:
    with st.spinner("Analyzing onsets..."):
        with temp_audio_file(audio_val) as temp_path:
            processor = AudioProcessor(temp_path)
            score, feedback, _ = evaluate_generic(processor, "onset")
        ...
        if st.button("Claim XP"):
            add_score("Exercise 1.3", score, 50)
